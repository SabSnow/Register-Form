if(window.desktopPage)
	desktopPage.wins = {
		active: null,
		setActiveStyle: function(winId){
			if(desktopPage.wins.active)
				webix.html.removeCss($$(desktopPage.wins.active).$view,"active_win");
			webix.html.addCss($$(winId).$view,"active_win",true);
			desktopPage.wins.active = winId;
		},
		...
		showEmptyApp: function(obj){
			...
			if(!$$(winId)){
				var c = desktopPage.wins.getVisibleWinCount();

				webix.ui({
					view:"window",
					id: winId,
					css:"popup-window-custom "+obj.$css||"",
					position: desktopPage.wins.getPosition,
		...
		showApp: function(name){
			...
			var c = desktopPage.wins.getVisibleWinCount();
			if(!$$(winId)){
				var config = desktopPage.wins.ui[name];
				webix.ui({
					view:"window",
					id: winId,
					...
					height:450,
					width:800,
					head:  desktopPage.wins.ui.toolbar.apply(this,config.toolbar()),
					body: config.body(),
					on: config.events
				});
		...
		ui:{
			...
			scheduler:{
				...
				events:{
					onBeforeShow: function(){
						desktopPage.beforeWinShow("scheduler");
					}
				}
			},
			gantt:{
				...
				events:{
					onBeforeShow: function(){
						desktopPage.beforeWinShow("gantt");
					}
				}
			},
			filemanager:{
				...
				events:{
					onBeforeShow: function() {
						desktopPage.beforeWinShow("filemanager");
					},
					onShow: function(){
