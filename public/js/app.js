desktopPage = {
		...
		webix.attachEvent("onFocusChange", function(view){
			if(view){
				...
				if(id.indexOf("_win") != -1){
					desktopPage.setActiveWindow(id);
				}
			}
		});
	},
					...
					cols:[
						{
							...
							on: {
								onItemDblClick: desktopPage.wins.showApp
							}
						},
						{}
					]
	...
	signOut: function(){
		desktopPage.wins.hideAllWindows();
		webix.$$("toolbar").hide();
		webix.$$("sign-in").show();
	},
									...
									on:{
										onItemClick: function(id){
											...
											if(id == "sign-out"){
												...
												if(desktopPage.fullScreen(!item.state)){
						...
						},
						{
							...
							rows:[
								...
								},
								{
									...
									on:{
										onItemClick: function(id){
											desktopPage.wins.showEmptyApp(this.getItem(id));
										}
									}
								}
							]
	...
	setActiveWindow: function(id){
		desktopPage.wins.setActiveStyle(id);
		var btn = $$(id.replace("win","button"));
		if(btn){
			desktopPage.deleteActiveBg();
